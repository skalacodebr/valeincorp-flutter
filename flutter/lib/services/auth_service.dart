  ...
  Future<ApiResponse<AuthResponse>> login(String email, String senha) async {
    try {
      ...
      if (e.toString().contains('DioException')) {
        ...
        if (statusCode == 401) {
          errorMessage = responseData?['message'] ?? 'Email ou senha incorretos';
        } else if (statusCode == 403) {
          errorMessage = responseData?['message'] ?? 'Conta não aprovada';
        } else if (statusCode == 422) {
          errorMessage = responseData?['message'] ?? 'Dados inválidos';
